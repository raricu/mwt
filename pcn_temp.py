...
dataloader = np.load('Data/temps/temps.npy')
...
def fourier(acc_rates_mean):
    ...
    checkpoint = torch.load('NS_models/temps_new/old.pt')
...
def pcn(y, n_iters, beta, sigma):
    ...
    u0 = np.random.normal(245, scale = 0.1)
    X = []
    acc = 0
    u_prev = u0
    ...
    for i in tqdm.trange(n_iters):
        u_new = np.sqrt(1 - pow(beta, 2)) * u_prev + beta * np.random.normal(0, scale = 0.1) # scale = std
...
# # # Load noisy data
y = np.load('Data/temps/temps_new.npy')
...
for k in range(y.shape[0]):
    ...
    np.save('simulations/temps_new/sim{}.npy'.format(k), pcn_u)
    print("Acceptance rate: ", pcn_acc)
simulations = np.array(simulations)
np.save('simulations/temps_new/sim_total.npy', simulations)